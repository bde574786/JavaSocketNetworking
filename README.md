...
### Review
네트워크를 배우기 전까지 소켓 통신의 동작 원리와 프로토콜을 이해하기까지 많은 어려움이 있었다.
또한, 서버와 클라이언트를 연결할 수 없거나 연결 시간 초과, 데이터 송수신 오류 등 각종 예외 사항을 다루는데 미숙하여 적절한 조치를 취할 수 없었다. 
예외 상황을 마주할 때마다 코드를 되돌리면서 코드의 흐름을 파악하는 데에 집중하였다. 
소켓 통신을 이해하는 데에는 충분한 프로젝트였으나 제한된 시간과 기술 부족 때문에 보안 요소가 빈약하여 아쉬움이 남는다.
